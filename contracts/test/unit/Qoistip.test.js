...
xdescribe('Qoistip', function () {
    ...
    describe('New user', async () => {
        it('Register new user and emit event NewUser', async () => {
            ...
            const userToken1Address = await qoistip.tokenUser(addr1.address);

            await expect(registerUserTransation)
                .to.emit(qoistip, 'NewUser')
                .withArgs(addr1.address, userToken1Address);
